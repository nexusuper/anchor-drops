...
// check() is async (the Upstash-backed path does a network round trip); this
// suite exercises the in-memory fallback, which resolves synchronously but
// still returns a Promise, so every call site here awaits.
...
for (let i = 0; i < 3; i++) {
  assert.equal(await dynamic(req(`/api/orders/id-${i}`), res()), true, `request ${i} should pass`);
}
const blocked = res();
assert.equal(await dynamic(req('/api/orders/id-999'), blocked), false);
assert.equal(blocked.code, 429);
assert.ok(Number(blocked.headers['Retry-After']) > 0);

// A query string is not a fresh bucket either.
assert.equal(await dynamic(req('/api/orders/id-0?phone=09171234567'), res()), false);

// Different limiter instance = different route handler = independent budget.
const other = rateLimit({ windowMs: 60_000, max: 3 });
assert.equal(await other(req('/api/orders/id-0'), res()), true);

// Different IP, same limiter = independent budget.
assert.equal(await dynamic(req('/api/orders/id-0', '5.6.7.8'), res()), true);
...
for (let i = 0; i < 1000; i++) {
  assert.equal(await dynamic(req(`/api/orders/${i}`), res()), false);
}

// The window expires and the budget comes back.
const short = rateLimit({ windowMs: 1, max: 1 });
assert.equal(await short(req('/api/x'), res()), true);
assert.equal(await short(req('/api/x'), res()), false);
