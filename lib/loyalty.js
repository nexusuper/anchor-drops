...
// Loyalty counts refills, not literal gallons: one delivered container = 1 point,
// so 10 delivered refills earn one voucher. (It used to count 5 per gallon
// container, which handed out a voucher every 2 refills.) The "gallons" names
// below are kept for API/UI field compatibility.
export const GALLONS_BY_SIZE = { '5-Gal': 1, '3-Gal': 1 };
export const VOUCHER_VALUE = 30;       // ₱ value of one free gallon refill
export const GALLONS_PER_VOUCHER = 10; // delivered refills needed to earn one voucher
...
// How many vouchers may be applied to one order: one voucher buys one refill,
// so the cap is what's available and how many refills are in the cart.
//
// This used to also divide the refill subtotal by VOUCHER_VALUE, which silently
// blocked redemption on the ₱25 store-pickup refill — one earned free refill
// could not be spent on a single refill, because 25 < 30. VOUCHER_VALUE is the
// peso value of a *delivery* refill, not a floor on what a voucher may buy.
// Callers clamp the discount to the subtotal so a voucher never pays out cash.
export function maxRedeemable({ available, quantity, refillSubtotal }) {
  if ((Number(refillSubtotal) || 0) <= 0) return 0;
  return Math.max(0, Math.min(Number(available) || 0, Number(quantity) || 0));
}
