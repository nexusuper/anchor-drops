...
// Entries that have actually been edited — same filter used to decide both
// what's shown (components/Testimonials.js) and what's claimed in schema
// (testimonialsSchema below), so the two can never disagree about whether a
// given entry counts as real.
export function realTestimonials(testimonials) {
  return testimonials.filter((t) => t.quote !== PLACEHOLDER_QUOTE);
}
