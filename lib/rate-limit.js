...
// Optional `keyBy(req)` throttles per something other than IP (e.g. the
// normalized phone being looked up, so one attacker can't enumerate phones from
// rotating IPs and a shared NAT can't block everyone). Falsy return → IP.
export function rateLimit({ windowMs = 60_000, max = 20, keyBy } = {}) {
  return function check(req, res) {
    const target = (keyBy && keyBy(req)) || getIp(req);
