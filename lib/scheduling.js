...
// Store is closed Sunday, unless admin flagged that date open via
// app_settings 'open_override_dates' (e.g. a one-off Sunday opening).
// Date-only string, parsed as UTC so weekday doesn't shift with the server's
// local timezone.
export function isStoreOpenDay(dateStr, openOverrides = []) {
  if (!isValidDate(dateStr)) return false;
  if (openOverrides.includes(dateStr)) return true;
...
// Next date the store is open, strictly after dateStr (skips Sunday, unless overridden open).
export function nextOpenDay(dateStr, openOverrides = []) {
  let d = addDays(dateStr, 1);
  while (!isStoreOpenDay(d, openOverrides)) d = addDays(d, 1);
  return d;
}

// Morning pickup: delivery locked to that same day's afternoon window
// (minDate === maxDate). Afternoon pickup: delivery can be the next open day
// or any later open day the customer wants (maxDate: null = unbounded).
export function computeAllowedDeliveryWindow({ pickupDate, pickupTime, openOverrides = [] }) {
  if (!isValidDate(pickupDate)) return null;
  const slot = classifyPickupTime(pickupTime);
  if (!slot) return null;
  if (slot === 'morning') {
    return { minDate: pickupDate, maxDate: pickupDate, minTime: DELIVERY_SAME_DAY_START, maxTime: DELIVERY_SAME_DAY_END };
  }
  return { minDate: nextOpenDay(pickupDate, openOverrides), maxDate: null, minTime: DELIVERY_NEXT_DAY_START, maxTime: DELIVERY_NEXT_DAY_END };
}
...
export function validateSchedule({ hasEmptyContainers, pickupDate, pickupTime, deliveryDate, deliveryTime, today, nowTime, openOverrides = [] }) {
  ...
  if (!hasEmptyContainers) {
    ...
    if (!isStoreOpenDay(deliveryDate, openOverrides)) return { ok: false, error: 'Store is closed Sundays — please choose Mon-Sat' };
  ...
  if (!isStoreOpenDay(pickupDate, openOverrides)) return { ok: false, error: 'Store is closed Sundays — please choose Mon-Sat' };
  ...
  const allowed = computeAllowedDeliveryWindow({ pickupDate, pickupTime, openOverrides });
  const deliveryDateOk =
    allowed &&
    deliveryDate >= allowed.minDate &&
    (allowed.maxDate === null || deliveryDate <= allowed.maxDate) &&
    isStoreOpenDay(deliveryDate, openOverrides);
  ...
  if (!allowed || !deliveryDateOk || !deliveryTimeOk) {
