  ...
  useEffect(() => {
    ...
    async function loadProfile(s) {
      ...
      const { data, error: err } = await supabase
        .from('profiles')
        .select('id, role, branch_id, full_name, avatar_url, is_active')
        .eq('id', s.user.id)
        .single();
      if (cancelled) return;
      setSession(s);
      if (err || !data) {
        ...
        // app_role() ignores inactive rows (migration 0031), so RLS grants this
        // session nothing — say so instead of rendering screens that read empty.
        setProfile(null);
        setError('This account is deactivated. Ask the owner to reactivate it.');
      } else {
        setProfile(data);
        setError(null);
      }
