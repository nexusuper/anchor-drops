...
export function strikeVerdict(strikes, paymentMethod) {
  if (strikes >= STRIKES_BLOCKED) {
    return {
      ok: false,
      error: 'We are unable to accept online orders from this number. Please message us on Facebook so we can help.',
    };
  }
  if (strikes >= STRIKES_PREPAY_ONLY && paymentMethod === 'cod') {
    return {
      ok: false,
      error: 'Cash on delivery is unavailable for this order. Please pay via GCash to continue.',
    };
  }
  return { ok: true };
}
